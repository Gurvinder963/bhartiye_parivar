...
class SeriesDetailPage extends StatefulWidget {
  ...
  SeriesDetailPage({Key key,@required this.content}) : super(key: key);
  @override
  SeriesDetailPageState createState() {

    videoCategory=content.videoCategory;
    return SeriesDetailPageState(content);
  }
}
class ReportList {
  String name;
...
class SeriesDetailPageState extends State<SeriesDetailPage> {
  ...
  void apiCall(){
    ...
    getLocaleContentLang().then((locale) {
      ...
      getVideosListSuggestion(user_Token,videoCategory,mContent.videoId.toString(),mContent.seriesId.toString()).then((value) => {
...
/*
  ...
  Future<SeriesListResponse> getVideosListSuggestion(String user_Token,String videoCategory,String videoId,String seriesId) async {
    MainRepository repository=new MainRepository();
    String pageIndex = page.toString();
    var body =json.encode({"appcode":Constants.AppCode, "token": user_Token,"userid": USER_ID,"video_category":videoCategory,"page":pageIndex,"video_id":videoId,"series_id":seriesId});


      return repository.fetchSeriesSuggestListJAVA(body);
  ...
  Future<AddToCartResponse> saveReportAPI(message) async {
    ...
    var body =json.encode({"content_id":mContent.videoId.toString(),"content_type":"1","message":message});
    MainRepository repository=new MainRepository();
                                          ...
                                          children: <Widget>[
                                            ...
                                            IconButton(
                                                ...
                                                onPressed: () {
                                                  ...
                                                  postAddLike("1",user_Token,mContent.videoId.toString())
                                                      .then((res) async {
                                                    String msg="";
                                                    if(likeStatus==1){
  ...
  Widget _buildBoxVideoList(BuildContext context,int index,Series seriesData){
